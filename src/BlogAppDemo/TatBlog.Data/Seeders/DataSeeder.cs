...
public class DataSeeder : IDataSeeder
{
	...
	public DataSeeder(BlogDbContext dbContext, IPasswordHasher<Account> passwordHasher)
	{
	...
	private Account AddRolesAndAccount()
	{
		...
		var editor = new Account()
		{
			FullName = "Phong Son",
			UserName = "phongson",
			NormalizedUserName = "PHONGSON",
			Email = "phong.son@gmail.com",
			NormalizedEmail = "PHONG.SON@GMAIL.COM",
			PhoneNumber = "0987654321",
			EmailConfirmed = true,
			PhoneNumberConfirmed = true,
			SecurityStamp = Guid.NewGuid().ToString("D")
		...
		if (!_dbContext.Set<Account>().Any(x => x.UserName == editor.UserName))
		{
			editor.PasswordHash = _passwordHasher.HashPassword(editor, "123456");

			_dbContext.Set<Account>().Add(editor);
			_dbContext.Set<UserInRole>().Add(new UserInRole()
			{
				Account = editor,
				Role = roles.First(x => x.Name == "Editor")
			});
		}
